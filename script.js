...
(function () {
  ...
  const backgroundImages = {
    "--story-background-image": story.storyBackgroundImage,
    "--pregnancy-background-image": story.pregnancyBackgroundImage,
    "--letter-background-image": story.letterBackgroundImage,
    "--gallery-background-image": story.galleryBackgroundImage,
    "--surprise-background-image": story.surpriseBackgroundImage
  };

  Object.entries(backgroundImages).forEach(([property, path]) => {
    if (path) document.documentElement.style.setProperty(property, `url("${path}")`);
  });
  ...
  const savedTheme = safeGet("baby-story-theme", story.defaultTheme || "dark");
  ...
  themeToggle.addEventListener("click", () => {
    ...
    safeSet("baby-story-theme", nextTheme);
  });
...
updateSongButton();
  ...
  function showMonth(index) {
    ...
    replaceChildren(
      "#monthVideo",
      createVideo(month.video, `${month.label} video`)
    );

    /* Safe magical transition for browsers that support it. */
    const monthPanel = $("#monthStory");

    if (monthPanel && typeof monthPanel.animate === "function") {
      monthPanel.getAnimations?.().forEach((animation) => {
        animation.cancel();
      });
  ...
  /* Create the month-navigation buttons. */
  story.pregnancyMonths.forEach((month, index) => {
    ...
    label.textContent =
      index === story.pregnancyMonths.length - 1
        ? "Hello"
        : `Month ${index + 1}`;
    ...
    button.addEventListener("keydown", (event) => {
      ...
      const next =
        (selectedMonth + direction + story.pregnancyMonths.length) %
        story.pregnancyMonths.length;
  ...
  /* Display Month One when the page first loads. */
  showMonth(Math.min(selectedMonth, story.pregnancyMonths.length - 1));
  /* ------------------------------------------------------------------
   TWO LETTERS FROM TWO SISTERS
------------------------------------------------------------------ */

const letterSection = story.letterSection || {};
const letters = Array.isArray(story.letters)
  ? story.letters
  : [];

setText("#lettersKicker", letterSection.kicker);
setText("#lettersTitle", letterSection.title);
setText(
  "#lettersIntroduction",
...
letters.forEach((letter, index) => {
  ...
  openButton.className =
    "envelope-button open-letter-button";
  ...
  openButton.innerHTML = `
    ...
    <span
      ...
      aria-hidden="true"
    >
      <strong class="letter-writer"></strong>
    </span>
    ...
    <span class="envelope-instruction">
  ...
  openButton.querySelector(
    ...
    letter.preview || `Letter ${index + 1}`;

  const paperShell = document.createElement("div");
  paperShell.className = "letter-paper-shell";
  paperShell.id = `letter-paper-${index}`;

  const paperInner = document.createElement("div");
  paperInner.className = "letter-paper-inner";

  const article = document.createElement("article");
  article.className =
    "opened-letter my-letter-card";

  const closeButton = document.createElement("button");
  closeButton.className = "close-letter";
  closeButton.type = "button";
  closeButton.textContent = "Close letter ×";

  const corner = document.createElement("div");
  corner.className = "letter-corner";
  ...
  writer.textContent = letter.from || "";
  ...
  paperInner.append(article);
  paperShell.append(paperInner);

  envelope.append(openButton, paperShell);
  letterItem.append(envelope);
  lettersGrid.append(letterItem);

  function setThisLetterOpen(open) {
    if (open) {
      $$(".letter-item", lettersGrid).forEach(
        (otherItem) => {
          if (otherItem === letterItem) return;

          otherItem.classList.remove("is-open");

          const otherEnvelope =
            $(".letter-envelope", otherItem);

          const otherButton =
            $(".open-letter-button", otherItem);

          otherEnvelope?.classList.remove("is-open");
    ...
    letterItem.classList.toggle("is-open", open);
  ...
  openButton.addEventListener("click", () => {
    setThisLetterOpen(true);
  });

  closeButton.addEventListener("click", () => {
    setThisLetterOpen(false);
  ...
  const revealNameButton = $("#revealName");
const nameCard = $("#nameCard");

revealNameButton.addEventListener("click", () => {
  /*
    The wand casts its spell first.
    The name appears when the spell reaches the button.
  */
  revealNameButton.disabled = true;

  nameCard.classList.remove("revealed");
  nameCard.classList.add("casting");

  window.setTimeout(() => {
    $("#nameHidden").hidden = true;
    $("#nameShown").hidden = false;

    nameCard.classList.remove("casting");
    nameCard.classList.add("revealed");

    revealNameButton.disabled = false;
  }, 900);
});
...
$("#hideName").addEventListener("click", () => {
  ...
  story.voices.forEach((voice) => {
    ...
    const audioPaths = Array.isArray(voice.audio)
      ? voice.audio.filter(Boolean)
      : voice.audio
        ? [voice.audio]
        : [];
    ...
    if (audioPaths.length) {
      ...
      recordings.className = "voice-recordings";

      audioPaths.forEach((audioPath, index) => {
        const recording = document.createElement("div");
        recording.className = "voice-recording";

        const label = document.createElement("small");
        label.textContent = `Grandpa’s message ${index + 1}`;
        ...
        audio.setAttribute(
          "aria-label",
          `Audio message ${index + 1} from ${voice.name}`
        );

        recording.append(label, audio);
        recordings.append(recording);
      });

      row.append(avatar, details, recordings);
    } else {
  ...
  $("#openSurprise").addEventListener("click", () => {
    ...
    if (playRequest && typeof playRequest.catch === "function") {
      playRequest.catch(() => {
        /* Controls remain visible if a browser requests one manual tap. */
      });
    }
  });
...
const numberOfSparkles = reduceMotion
  ? 0
...
document.addEventListener("pointerdown", function (event) {
  ...
  const burstAmount = window.matchMedia("(max-width: 580px)").matches ? 5 : 7;
